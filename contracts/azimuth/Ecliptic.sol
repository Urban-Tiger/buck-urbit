...
contract Ecliptic is EclipticBase, SupportsInterfaceWithLookup, ERC721Metadata {
    ...
    function ownerOf(
        uint256 _tokenId
    ) public view validPointId(_tokenId) returns (address owner) {
    ...
    function getApproved(
        uint256 _tokenId
    ) public view validPointId(_tokenId) returns (address approved) {
    ...
    function tokenURI(
        uint256 _tokenId
    ) public view validPointId(_tokenId) returns (string _tokenURI) {
    ...
    //
    function transferPoint(uint32 _point, address _target, bool _reset) public {
    ...
    //
    function getSpawnLimit(
        uint32 _point,
        uint256 _time
    ) public view returns (uint32 limit) {
    ...
    //
    function canEscapeTo(
        uint32 _point,
        uint32 _sponsor
    ) public view returns (bool canEscape) {
