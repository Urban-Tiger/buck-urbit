...
contract Polls is Ownable {
    ...
    //                           on the _proposal
    //
    function hasVotedOnUpgradePoll(
        uint8 _galaxy,
        address _proposal
    ...
    //                            on the _proposal
    //
    function hasVotedOnDocumentPoll(
        uint8 _galaxy,
        bytes32 _proposal
    ) external view returns (bool result) {
    ...
    //
    function processVote(Poll storage _poll, uint8 _as, bool _vote) internal {
    ...
    //
    function updateUpgradePoll(
        address _proposal
    ) public onlyOwner returns (bool majority) {
        //  _proposal must not have achieved majority before
        //
        require(!upgradeHasAchievedMajority[_proposal]);
    ...
    //
    function updateDocumentPoll(
        bytes32 _proposal
    ) public returns (bool majority) {
        //  _proposal must not have achieved majority before
        //
        require(!documentHasAchievedMajority[_proposal]);
    ...
    //
    function checkPollMajority(
        Poll _poll
    ) internal view returns (bool majority) {
